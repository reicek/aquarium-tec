...
- PSU side: split rear structure rather than a full wall, using two fixed-hole Mean Well LRS-350-24 side-mount uprights aligned to the 150 mm long-axis hole pitch, a narrow center driver spine, a rear-facing Double BTS7960 mount with the heatsink outboard, and a compact tie-slot landing on the lowest part of that same spine for an external 24 V to 5 V logic buck below the driver; keep the large pass-through window above the buck zone and preserve the widened lower cable notch for the capacitor and cable exit
